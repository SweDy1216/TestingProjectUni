...
test.describe.serial("Check specialty functionality", () => {
  ...
  test("Create specialty", async () => {
    const testSpecialtyName = "testKholodnitskyi";
    ...
    await test.step("Save added values", async () => {
      await pages.getSpecialtyListPage().saveButton().click();
    });

    await test.step(
      `Verify that new specialty in the list contains '${testSpecialtyName}'`,
      async () => {
  ...
  test("Edit specialty", async () => {
    const newTestSpecialtyName = "testKholodnitskyiNew";
    await test.step(
    ...
    await test.step(
      `Change name to '${newTestSpecialtyName}'`,
      async () => {
        await page.waitForTimeout(1000);
        await pages
    ...
    await test.step("Verify that specialty data was updated", async () => {
      expect(
  ...
  test("Delete specialty", async () => {
    ...
    await test.step(
      "Get the number of specialties in the list before deleting",
      async () => {
        await page.waitForTimeout(1000);
    ...
    await test.step(
      'Select the last specialty in the list and click "Delete" button',
      async () => {
    ...
    await test.step(
      "Verify that the count of items was changed 1 after delete",
      async () => {
        await page.waitForTimeout(1000);
