...
test.describe.serial("Check vets functionality", () => {
  ...
  test("Create vet", async () => {
    const firstNameVet = "Vladyslav";
    const lastNameVet = "Kholodnitskyi";
  ...
  test("Delete vet", async () => {
    ...
    await test.step(
      "Get the number of vets in the list before deleting",
      async () => {
        await page.waitForTimeout(1000);
    ...
    await test.step(
      'Select the last vet in the list and click "Delete" button',
      async () => {
    ...
    await test.step(
      "Verify that the count of items was changed after 1 delete",
      async () => {
        await page.waitForTimeout(1000);
